...
import countriesReducer from '../../slices/countriesSlice'; // Правильный импорт
...
const renderWithStore = (users: RegisteredUser[] = [], countries: string[] = []) => {
  const store = configureStore({
    reducer: {
      registeredUser: registeredUsersReducer, // редюсер для пользователей
      country: countriesReducer, // редюсер для стран
    },
    preloadedState: {
      registeredUser: { registeredUsers: users },
      country: { countries }, // состояние для стран
    },
  });
