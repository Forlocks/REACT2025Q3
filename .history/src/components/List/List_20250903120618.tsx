...
export const List: React.FC<ListProps> = ({
  ...
  }, [country, year, sortAttribute, sortOrder, emissions]);

  // 🟢 useCallback для обработчиков
  const renderColumn = useCallback(
    (id: string, countryAttributes: EmissionAttributes) => {
      return <td key={id}>{formatNumber(countryAttributes[id])}</td>;
    },
    []
  );
        ...
        {Object.entries(processedEmissions).map(([countryName, countryData]) => {
          ...
          return (
            <tr key={countryName}>
              ...
              {additionColumns.map(id => renderColumn(id, countryAttributes))}
            </tr>
          );
        })}
      </tbody>
