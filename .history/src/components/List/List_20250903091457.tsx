    ...
    <table>
      ...
      <tbody>
        {Object.entries(emissions).map(([countryName, countryData]) => {
          const coun = countryData.data.find((item: EmissionEntry) => item.year === year);
          if (!coun) return null;
          ...
          return (
            <tr key={countryName}>
              ...
              <td>{coun.year}</td>
              <td>{coun.population ?? PLACEHOLDER_TEXT}</td>
              <td>{formatNumber(coun.co2)}</td>
              <td>{formatNumber(coun.co2_per_capita)}</td>
              {additionColumns.map(id => (
                <td key={id}>{formatNumber(coun[id])}</td>
              ))}
            </tr>
          );
        })}
