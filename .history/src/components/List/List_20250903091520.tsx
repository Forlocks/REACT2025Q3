    ...
    <table>
      ...
      <tbody>
        {Object.entries(emissions).map(([countryName, countryData]) => {
          const countryAtr = countryData.data.find((item: EmissionEntry) => item.year === year);
          if (!countryAtr) return null;

          return (
            <tr key={countryName}>
              <td>{countryName}</td>
              <td>{countryData.iso_code ?? PLACEHOLDER_TEXT}</td>
              <td>{countryAtr.year}</td>
              <td>{countryAtr.population ?? PLACEHOLDER_TEXT}</td>
              <td>{formatNumber(countryAtr.co2)}</td>
              <td>{formatNumber(countryAtr.co2_per_capita)}</td>
              {additionColumns.map(id => (
                <td key={id}>{formatNumber(countryAtr[id])}</td>
              ))}
            </tr>
          );
        })}
