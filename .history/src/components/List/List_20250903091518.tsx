    ...
    <table>
      ...
      <tbody>
        {Object.entries(emissions).map(([countryName, countryData]) => {
          const countryAyy = countryData.data.find((item: EmissionEntry) => item.year === year);
          if (!countryAyy) return null;

          return (
            <tr key={countryName}>
              <td>{countryName}</td>
              <td>{countryData.iso_code ?? PLACEHOLDER_TEXT}</td>
              <td>{countryAyy.year}</td>
              <td>{countryAyy.population ?? PLACEHOLDER_TEXT}</td>
              <td>{formatNumber(countryAyy.co2)}</td>
              <td>{formatNumber(countryAyy.co2_per_capita)}</td>
              {additionColumns.map(id => (
                <td key={id}>{formatNumber(countryAyy[id])}</td>
              ))}
            </tr>
          );
        })}
