    ...
    <table>
      ...
      <tbody>
        {Object.entries(emissions).map(([countryName, countryData]) => {
          const country = countryData.data.find((item: EmissionEntry) => item.year === year);
          ...
          return (
            <tr key={countryName}>
              ...
              <td>{country.year}</td>
              <td>{country.population ?? PLACEHOLDER_TEXT}</td>
              <td>{formatNumber(country.co2)}</td>
              <td>{formatNumber(country.co2_per_capita)}</td>
              {additionColumns.map(id => (
                <td key={id}>{formatNumber(country[id])}</td>
              ))}
            </tr>
          );
        })}
